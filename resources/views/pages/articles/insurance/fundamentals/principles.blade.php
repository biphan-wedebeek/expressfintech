...
@section('extra_badges')
    <span class="bg-white/10 text-navy text-[11px] font-medium px-3 py-1.5 rounded-lg border border-white/10">Insurance Fundamentals</span>
@endsection

@section('sidebar_toc')
    <li><a href="#intro"    class="text-[13px] text-[#888] hover:text-[#4f46e5] transition-colors">Introduction</a></li>
    <li><a href="#pooling"  class="text-[13px] text-[#888] hover:text-[#4f46e5] transition-colors">Risk Pooling</a></li>
    <li><a href="#capital"  class="text-[13px] text-[#888] hover:text-[#4f46e5] transition-colors">Capital Structure</a></li>
    <li><a href="#metrics"  class="text-[13px] text-[#888] hover:text-[#4f46e5] transition-colors">Industry Metrics</a></li>
    <li><a href="#outlook"  class="text-[13px] text-[#888] hover:text-[#4f46e5] transition-colors">Strategic Outlook</a></li>
@endsection

@section('page_content')

<section id="intro" class="mb-20 scroll-mt-28">
    <h2 class="familyfont text-[28px] font-bold border-l-4 border-[#4f46e5] pl-4 mb-6">
        Introduction
    </h2>
    <p class="text-[#6b7280] text-[15.5px] leading-[1.85] mb-5">
        Insurance markets redistribute financial uncertainty across diversified capital pools. At its core, insurance is a mechanism of <strong class="text-navy">risk transfer</strong> — policyholders exchange a certain, smaller cost (the premium) for protection against an uncertain, potentially catastrophic loss.
    </p>
    <p class="text-[#6b7280] text-[15.5px] leading-[1.85] mb-5">
    ...
    <div class="grid grid-cols-3 gap-5 mt-10">
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] p-6 text-center">
            <div class="text-[36px] font-extrabold text-[#4f46e5] mb-2" style="font-family:'Playfair Display',serif">{{ $m[0] }}</div>
...
<div class="bg-white rounded-2xl border border-[#e5e7eb] p-14 mb-20 relative overflow-hidden">
    <h3 class="familyfont text-[26px] font-bold mb-4">
        Structural Transformation in Insurance
    </h3>
    <p class="text-[#6b7280] max-w-[680px] text-[15px] leading-relaxed">
...
<section id="pooling" class="mb-20 scroll-mt-28">
    ...
    <div class="rounded-xl border border-[#e5e7eb] overflow-hidden mb-8">
        <div class="px-6 py-3 border-b border-[#e5e7eb]" style="background:#f8f9fb">
            <span class="text-[11px] font-bold uppercase tracking-[0.12em] text-[#4f46e5]">Risk Pooling Mechanics</span>
        </div>
        <div class="divide-y divide-[#e5e7eb]">
            ...
            <div class="px-6 py-4 flex items-start gap-5" style="background:#ffffff">
                <div class="w-[180px] shrink-0 text-[13.5px] font-semibold text-[#4f46e5]">{{ $row[0] }}</div>
...
<section id="capital" class="mb-20 scroll-mt-28">
    ...
    <div class="space-y-3 mb-8">
        @foreach([
            ['Tier 1 Capital', '100%', 'bg-[#4f46e5]', 'Common equity, retained earnings — highest quality, fully loss-absorbing'],
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] p-5 flex items-center gap-5">
            <div class="w-[120px] shrink-0 text-[13px] font-bold text-navy">{{ $tier[0] }}</div>
            <div class="flex-1 bg-[#f8f9fb] rounded-full h-2.5 overflow-hidden">
                <div class="h-full rounded-full {{ $tier[2] }}" style="width:{{ $tier[1] }}"></div>
            </div>
            <div class="w-10 text-right text-[13px] font-bold text-[#4f46e5]">{{ $tier[1] }}</div>
...
<section id="metrics" class="mb-20 scroll-mt-28">
    ...
    <div class="rounded-xl border border-[#e5e7eb] overflow-hidden">
        <div class="px-6 py-3 border-b border-[#e5e7eb]" style="background:#f8f9fb">
            <span class="text-[11px] font-bold uppercase tracking-[0.12em] text-[#4f46e5]">Key Performance Indicators — Property & Casualty Sector 2025</span>
        </div>
            ...
            <table class="w-full text-[13.5px]">
                ...
                <tbody class="divide-y divide-[#e5e7eb]">
                    ...
                    <tr style="background:#ffffff" class="hover:bg-[#f0f1f5] transition-colors">
                        ...
                        <td class="px-6 py-4 text-right font-bold text-[#4f46e5]">{{ $row[1] }}</td>
...
<section id="outlook" class="scroll-mt-28">
    ...
    <div class="grid grid-cols-1 gap-4">
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] p-6 flex items-start gap-5">
            <div class="w-2 h-2 rounded-full bg-[#4f46e5] shrink-0 mt-2"></div>
            <div>
                <h4 class="text-[15px] font-bold text-[#4f46e5] mb-2">{{ $item[0] }}</h4>
                <p class="text-[13.5px] text-[#6b7280] leading-relaxed">{{ $item[1] }}</p>
            </div>
        </div>
        @endforeach
    </div>
...
@section('insight_cards')
    ...
    <div class="bg-white rounded-2xl border border-[#e5e7eb] p-10 transition-all duration-300 hover:-translate-y-2 hover:border-[#4f46e5] cursor-pointer">
        <h4 class="familyfont text-[17px] font-bold mb-4">{{ $card[0] }}</h4>
